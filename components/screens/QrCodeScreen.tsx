  ...
  return (
    <div className="w-full max-w-sm mx-auto flex flex-col h-full bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 rounded-2xl shadow-2xl overflow-hidden animate-fade-in">
      ...
      <div className="flex-grow flex flex-col items-center justify-center p-8">
        <h2 className="text-2xl font-bold mb-4">{t('batchRegistered')}</h2>
        <div className="p-4 bg-white rounded-lg shadow-inner" ref={qrRef}>
          <QRCodeCanvas value={JSON.stringify({ batchId: batchDetails.batchId, hash: batchDetails.blockchainHash })} size={220} />
        </div>
        <div className="w-full mt-6 p-3 bg-gray-100 dark:bg-gray-700 rounded-lg border">
            <p className="text-xs font-semibold text-gray-500">{t('blockchainId')}</p>
            <p className="font-mono text-xs break-all">{batchDetails.blockchainHash}</p>
        </div>
      </div>
      <div className="p-6 border-t grid grid-cols-2 gap-4">
          <Button variant="outline" onClick={handleDownload}><DownloadIcon className="h-5 w-5 mr-2" />{t('downloadQrCode')}</Button>
          <Button variant="secondary" onClick={handleDone}>{t('done')}</Button>
      </div>
    </div>
  );
};
