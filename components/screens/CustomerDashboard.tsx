  ...
  return (
    <div className="w-full max-w-sm mx-auto flex flex-col h-full bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 rounded-2xl shadow-2xl overflow-hidden animate-fade-in">
      ...
      <div className="flex-grow flex flex-col items-center justify-center text-center p-8">
        <p className="text-gray-800 dark:text-gray-400 mb-6">{t('scanProductPrompt')}</p>
        ...
        <div className="w-full mt-8"><Button variant="secondary" onClick={handleScan} disabled={isScanning}>{isScanning ? <Spinner /> : t('simulateScan')}</Button></div>
      </div>
    </div>
  );
};
