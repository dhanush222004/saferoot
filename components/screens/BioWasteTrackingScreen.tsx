    ...
    <div className="w-full max-w-sm mx-auto flex flex-col h-full bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 rounded-2xl shadow-2xl overflow-hidden animate-fade-in">
      ...
      <div className="flex-grow p-6 overflow-y-auto">
        <div className="p-4 bg-gray-50 dark:bg-gray-700/50 border dark:border-gray-600 rounded-lg mb-6">
            ...
            <p className="text-sm text-gray-800 dark:text-gray-400">{bioWasteSubmission.quantity}kg of {bioWasteSubmission.type}</p>
            <p className="text-sm text-gray-800 dark:text-gray-400">From: {bioWasteSubmission.location}</p>
        </div>
      ...
      {currentStatusIndex === STATUSES.length - 1 && (
        <div className="p-6 border-t border-gray-200 dark:border-gray-700 animate-fade-in"><Button variant="secondary" onClick={handleDone}>{t('done')}</Button></div>
      )}
    </div>
  );
};
